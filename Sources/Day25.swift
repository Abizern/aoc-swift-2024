...
struct Day25: AdventDay, Sendable {
  ...
  func part1() async throws -> Int {
    let keys = inputs.filter { if case .key = $0 { return true } else { return false } }
    let locks = inputs.filter { if case .lock = $0 { return true} else { return false} }
    var counts = 0
    for key in keys {
      for lock in locks {
        if lock.fits(key) {
...
extension Day25 {
  enum FivePin: Equatable {
    ...
    func fits(_ key: FivePin) -> Bool {
      guard case let .lock(lPins) = self,
            case let .key(kPins) = key else {
        return false
      }
